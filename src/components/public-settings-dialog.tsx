  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="h-9 rounded-md border-border bg-muted/30 hover:bg-accent text-muted-foreground font-semibold gap-2 text-xs uppercase tracking-wide px-4">
        ...
        <div className="flex flex-col md:flex-row h-full">
          ...
          <div className="md:w-72 bg-card/50 p-8 flex flex-col items-center justify-center text-center space-y-6 border-b md:border-b-0 md:border-r border-border">
             ...
             <div className="space-y-3">
                <div className="flex items-center justify-center gap-2 text-primary font-semibold uppercase tracking-widest text-[10px]">
                   <ShieldCheck className="h-3.5 w-3.5" />
                   {"សកម្មភាពសុវត្ថិភាព"}
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed px-2">
                  {"ភ្ញៀវអាចស្កែនកូដ ដើម្បីមើលព័ត៌មាន។"} {passcode ? "ការការពារដោយលេខសម្ងាត់កំពុងដំណើរការ។" : "ការចូលមើលជាសាធារណៈ។"}
                </p>
             </div>
             <Button variant="outline" size="sm" onClick={copyLink} className="w-full h-9 rounded-md border-border bg-muted/30 hover:bg-accent text-muted-foreground font-semibold text-xs transition-all">
          ...
          <div className="flex-1 p-8 overflow-y-auto max-h-[85vh]">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-lg font-bold text-foreground italic">{"ការកំណត់ការមើលជាសាធារណៈ"}</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-3">
                 <div className="flex items-center justify-between">
                    <Label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{"លេខសម្ងាត់ (PIN)"}</Label>
                    {passcode && <span className="text-[10px] font-semibold text-emerald-500 uppercase tracking-widest">{"បានចាក់សោ"} 🔐</span>}
                 </div>
                 <div className="relative group">
                    ...
                    <Input
                      ...
                      className="h-10 pl-10 pr-12 text-base tracking-[0.3em] font-bold rounded-md border-border bg-muted/30 text-foreground placeholder:text-muted-foreground/30 focus:ring-primary/20"
                    />
                    <button
                      type="button"
                 ...
                 <p className="text-[10px] text-muted-foreground/50 italic">{"លេខសម្ងាត់ ៤ ខ្ទង់ដើម្បីការពារទិន្នន័យភ្ញៀវ។"}</p>
              </div>

              {/* Guest List Visibility */}
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-md border border-border">
                <div className="space-y-0.5">
                  <Label className="text-sm font-semibold text-foreground">{"បង្ហាញបញ្ជីឈ្មោះភ្ញៀវ"}</Label>
                  <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-widest">{"ការមើលឃើញនៅលើជញ្ជាំងសាធារណៈ"}</p>
                </div>
                <Switch
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {/* Banner */}
                 <div className="space-y-3">
                    <Label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{"រូបភាព Banner (មិនមានក៏បាន)"}</Label>
                    <div
                    ...
                    >
                        {bannerPreview ? (
                            ...
                            <div className="flex flex-col items-center gap-2 opacity-30 text-foreground">
                                ...
                                <span className="text-[10px] font-semibold uppercase">{"រូបភាព Banner"}</span>
                            </div>
                        )}
                 ...
                 {/* Bank QR */}
                 <div className="space-y-3">
                    <Label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{"QR កូដ សម្រាប់ទទួលចំណងដៃ"}</Label>
                    <div
                    ...
                    {qrPreview && qrPreview !== "" ? (
                        ...
                        <span className="text-[10px] font-semibold uppercase">{"បញ្ចូលរូបភាព"}</span>
                        </div>
                    )}
              ...
              {/* Gallery Manager */}
              <div className="space-y-3">
                 <Label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{"អាល់ប៊ុមរូបភាពកម្មវិធី"}</Label>
                 <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 p-3 bg-accent/40 rounded-md border border-border">
                    {currentGalleryUrls.map((url, idx) => (
                        <div key={`cur-${idx}`} className="relative aspect-square rounded-md overflow-hidden border border-border group">
                            <img src={url} className="w-full h-full object-cover" />
                            <button onClick={() => removeCurrentGallery(idx)} className="absolute top-1 right-1 bg-background/60 text-foreground rounded-full p-0.5 opacity-0 group-hover:opacity-100 hover:bg-primary hover:text-white transition-all">
                                <X className="h-3 w-3" />
                            </button>
                        </div>
                    ))}
                    {newGalleryPreviews.map((pre, idx) => (
                        <div key={`new-${idx}`} className="relative aspect-square rounded-md overflow-hidden border border-rose-500/30 group">
                            <img src={pre} className="w-full h-full object-cover" />
                            <button onClick={() => removeNewGallery(idx)} className="absolute top-1 right-1 bg-background/60 text-foreground rounded-full p-0.5 opacity-0 group-hover:opacity-100 hover:bg-primary hover:text-white transition-all">
                                <X className="h-3 w-3" />
                            </button>
                            <div className="absolute bottom-1 right-1 bg-primary text-[6px] text-white px-1 rounded uppercase font-bold">New</div>
                        </div>
                    ))}
                    <button
            ...
            <div className="mt-8 flex gap-3">
              <Button variant="ghost" className="h-10 rounded-md font-semibold flex-1 text-muted-foreground hover:text-foreground hover:bg-accent transition-all" onClick={() => setOpen(false)}>{"បោះបង់"}</Button>
              <Button className="h-10 rounded-md font-bold flex-1 bg-primary hover:bg-primary/90 text-primary-foreground shadow-md transition-colors" onClick={handleSave} disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : "រក្សាទុកការផ្លាស់ប្តូរ"}
              </Button>
            </div>
          </div>
        </div>
