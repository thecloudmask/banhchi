...
export function AdminEventStats({ guests }: AdminEventStatsProps) {
  ...
  const bankPercentage =
    stats.counts.total > 0 ? (stats.counts.bank / stats.counts.total) * 100 : 0;
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-1">
            <h3 className="text-4xl font-black">
              {formatUsd(stats.usd.total)}
            </h3>
            <p className="text-xl font-bold opacity-60 font-mono">
              {formatKhr(stats.khr.total)} ៛
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-white/10 flex items-center justify-between">
            ...
            <div className="flex -space-x-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-6 w-6 rounded-full border-2 border-zinc-900 bg-zinc-800 flex items-center justify-center"
                >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h4 className="text-lg font-black text-gray-900">
                  {stats.counts.cash} {"នាក់"}
                </h4>
              </div>
            </div>
            <div className="space-y-1 pl-13">
              <p className="text-2xl font-black text-gray-900">
                {formatUsd(stats.usd.cash)}
              </p>
              <p className="text-sm font-bold text-gray-400 font-mono">
                {formatKhr(stats.khr.cash)} ៛
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h4 className="text-lg font-black text-gray-900">
                  {stats.counts.bank} {"នាក់"}
                </h4>
              </div>
            </div>
            <div className="space-y-1 pl-13">
              <p className="text-2xl font-black text-gray-900">
                {formatUsd(stats.usd.bank)}
              </p>
              <p className="text-sm font-bold text-gray-400 font-mono">
                {formatKhr(stats.khr.bank)} ៛
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-3">
          <div className="flex items-center justify-between text-[10px] font-black uppercase">
            <span className="text-orange-500">
              {"សាច់ប្រាក់"} {Math.round(100 - bankPercentage)}%
            </span>
            <span className="text-blue-500">
              {"ធនាគារ"} {Math.round(bankPercentage)}%
            </span>
          </div>
