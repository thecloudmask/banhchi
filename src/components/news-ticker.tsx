...
export function NewsTicker({ items }: NewsTickerProps) {
  ...
  return (
    <div className="bg-primary text-primary-foreground h-11 flex items-center overflow-hidden border-b border-primary-foreground/10 relative z-50">
      {/* Label/Badge */}
      <div className="bg-primary px-6 h-full flex items-center gap-3 z-10 border-r border-primary-foreground/10">
        ...
        <span
          className={cn(
            "text-[10px] font-black uppercase text-white whitespace-nowrap",
            "font-kantumruy text-[10px] font-bold",
          )}
        >
        ...
        <div className="animate-ticker flex items-center">
          {displayItems.map((item, idx) => {
            ...
            return (
              ...
              >
                <span className="text-[11px] text-white/50 uppercase font-black leading-none">
                  <span>{formatDateTime(date)}</span>
                </span>

                <span
                  className={cn(
                    "text-xs font-bold text-white transition-all group-hover:scale-[1.02] whitespace-nowrap",
