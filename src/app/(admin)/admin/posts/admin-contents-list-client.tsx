      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground uppercase">
            {"គ្រប់គ្រងអត្ថបទ និងខ្លឹមសារដែលបានផ្សព្វផ្សាយ"}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <Button
            ...
            className="h-10 rounded-md cursor-pointer px-6 font-bold text-xs uppercase bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/10 transition-all active:scale-95"
          >
        ...
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-3 pb-20">
          {contents.length === 0 ? (
            ...
            filteredContents.map((content) => (
              <div
                key={content.id}
                className="group border border-border bg-card/40 hover:bg-card rounded-md overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 animate-in fade-in slide-in-from-bottom-2"
              >
                <div
                ...
                >
                  ...
                  (content.images && content.images.length > 0) ? (
                    <Image
                      src={compressImage(
                        content.thumbnail || content.images![0],
                        "thumbnail",
                      )}
                      alt={content.title}
                      fill
                  ...
                  <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-20">
                    <span
                      className={cn(
                        "px-2.5 py-1 rounded-md text-[9px] font-black uppercase shadow-lg backdrop-blur-md border",
                        content.type === "wedding"
                          ? "bg-rose-500/20 text-rose-600 border-rose-500/30 dark:bg-rose-600/30 dark:text-rose-200"
                          : content.type === "buddhist"
                            ? "bg-orange-500/20 text-orange-600 border-orange-500/30 dark:bg-orange-600/30 dark:text-orange-200"
                            : content.type === "news"
                              ? "bg-amber-500/20 text-amber-600 border-amber-500/30 dark:bg-amber-500/30 dark:text-amber-200"
                              : content.type === "update"
                                ? "bg-blue-500/20 text-blue-600 border-blue-500/30 dark:bg-blue-600/30 dark:text-blue-200"
                                : content.type === "poster"
                                  ? "bg-pink-500/20 text-pink-600 border-pink-500/30 dark:bg-pink-600/30 dark:text-pink-200"
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-border">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-muted-foreground/40 uppercase mb-0.5">
                        ចេញផ្សាយ
                      </span>
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 rounded-md border border-border cursor-pointer text-muted-foreground hover:text-rose-500 hover:bg-rose-500/10 transition-all"
                          >
                        ...
                        <AlertDialogContent className="rounded-md border-border bg-card text-foreground p-8 shadow-2xl animate-in zoom-in duration-300">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className="text-muted-foreground text-sm pt-2">
                              ...
                              វានឹងលុបមាតិកានេះជាអចិន្ត្រៃយ៍ពីប្រព័ន្ធ។
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter className="mt-8 gap-3">
                            <AlertDialogCancel className="h-10 px-6 rounded-md font-bold text-xs bg-muted/30 border-border hover:bg-accent text-muted-foreground transition-all">
                              បោះបង់
                            </AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() =>
                              ...
                              className="h-10 px-8 rounded-md bg-destructive hover:bg-destructive/90 text-white font-bold text-xs shadow-lg shadow-destructive/20 transition-all active:scale-95"
                            >
                              លុបចេញ
                            </AlertDialogAction>
                          </AlertDialogFooter>
