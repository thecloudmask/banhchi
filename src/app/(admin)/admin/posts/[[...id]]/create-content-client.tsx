    ...
    <div className="min-h-screen bg-background text-foreground -m-4 sm:-m-8 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto space-y-12 pb-20">
        <div className="flex items-center gap-6">
          ...
          <div>
             <h1 className="text-4xl font-black tracking-tight text-foreground mb-2">បង្កើតអត្ថបទថ្មី</h1>
             <p className="text-muted-foreground font-bold uppercase tracking-[0.2em] text-[10px]">បំពេញព័ត៌មានខាងក្រោមដើម្បីផ្សព្វផ្សាយរឿងរ៉ាវ ឬសេចក្តីជូនដំណឹង</p>
          </div>
        </div>
        ...
        <div className="grid gap-12">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                          ...
                          {currentImageUrls.map((url, index) => (
                              <div key={`current-${index}`} className="relative aspect-square bg-muted rounded-md overflow-hidden border border-border group shadow-md">
                                  ...
                                  <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md text-[8px] text-white px-2 py-0.5 rounded-full uppercase font-black border border-white/10">Event</div>
                          ...
                          {imagePreviews.map((preview, index) => (
                              <div key={`new-${index}`} className="relative aspect-square bg-muted rounded-md overflow-hidden border border-border group shadow-md">
                                  <Image src={preview} alt={`Preview ${index}`} fill className="object-cover group-hover:scale-110 transition-transform duration-500" />
                                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                      <button 
                                          type="button"
                                  ...
                                  <div className="absolute top-3 left-3 bg-primary text-[8px] text-primary-foreground px-2 py-0.5 rounded-full uppercase font-black shadow-md">{currentImageUrls.length === 0 && index === 0 ? "Thumbnail" : "New"}</div>
                              </div>
                          ))}
          ...
          <div className="flex justify-end gap-6 pt-10 pb-20">
               ...
               >
                ...
                >
                   {loading ? <Loader2 className="h-7 w-7 animate-spin mr-3 text-white" /> : null}
